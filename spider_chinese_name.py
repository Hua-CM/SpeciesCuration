...
def get_chinese_name(latin_name):
    query_url = "http://www.iplant.cn/info/" + latin_name.strip()
    ...
    try:
        ...
        synom_name = a('.infomore').text()
        if '正名为' in synom_name:
            synom_url = a('.infomore a').attr('href')
            query_url = "http://www.iplant.cn/" + synom_url
            a = pq(get_html(query_url))
            try:
                chinese_name_inner = re.match("[\u4e00-\u9fa5]+", a('head>title').text()).group(0)
                return chinese_name_inner
